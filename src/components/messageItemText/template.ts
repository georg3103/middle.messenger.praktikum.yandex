export default
`<li class="message-item-text message-item-text--{{type}}">
    <div class="message-item-text__wrapper">
    {{#if isDataTime }}
        <time class="message-item-text__date-time">{{time}}</time>
    {{else}}
        <div class="message-item-text__text">
            {{message}}
            <div class="message-item-text__data data">
                {{#if seen }}
                <div class="data__status">
                    <svg width="11" height="5" viewBox="0 0 11 5" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <line y1="-0.5" x2="3.765" y2="-0.5" transform="matrix(0.705933 0.708278 -0.705933 0.708278 0.700195 2.33313)" stroke="#3369F3"/>
                        <line y1="-0.5" x2="5.6475" y2="-0.5" transform="matrix(0.705933 -0.708278 0.705933 0.708278 3.35828 5)" stroke="#3369F3"/>
                        <line y1="-0.5" x2="5.6475" y2="-0.5" transform="matrix(0.705933 -0.708278 0.705933 0.708278 6.01587 5)" stroke="#3369F3"/>
                    </svg>
                </div>
                {{/if}}
                <time class="message-item-text__time">{{time}}</time>
            </div>
        </div>
    {{/if}}
    </div>
</li>`;
